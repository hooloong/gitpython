...
def main():
    for database_name, conn_string, create_table_sql in database_strings:
        ...
        cur = conn.cursor()
        cur.execute(
            u"insert into pypyodbc_test_tabl values(1,'Hello! 这是pypyodbc模块',12.3,1234.55,'2012-11-11','17:31:32','2012-11-11',NULL, ?)",
            (binary_logo,))
        ...
        if database_name in ['Access', 'MySQL']:
            ...
            cur.execute(
                u"""select * from pypyodbc_test_tabl;update pypyodbc_test_tabl set kong = 5 where ID = 2;select ID, kong, riqi, product_name from pypyodbc_test_tabl where ID = 2 """)
            print cur.description
